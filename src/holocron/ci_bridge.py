...
def _push_pr_head(repo_dir, repo, number, source_branch, gitlab_provider):
    ...
    refspec = f"+refs/pull/{int(number)}/head:refs/heads/{source_branch}"
    try:
        subprocess.run(
...
def _await_pipeline_result(gitlab_provider, project_id, mr_iid, head_sha,
                           interval, timeout):
    """
    Waits for the MR's pipeline to finish.

    Returns (github_state, web_url, description). Phase 1 waits for the pipeline
    to appear (creation lags MR creation); phase 2 polls it to a terminal state.
    ...
    # Phase 1: locate the pipeline. Prefer the MR's own pipelines (robust to
    # merged-results pipelines whose SHA is a synthetic merge commit); fall back
    # to a SHA lookup.
    pipeline = None
    while pipeline is None:
        pipeline = (gitlab_provider.get_latest_mr_pipeline(project_id, mr_iid)
                    or gitlab_provider.get_pipeline_for_sha(project_id, head_sha))
        if pipeline is None:
